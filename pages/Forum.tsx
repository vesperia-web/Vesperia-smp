...
const Forum: React.FC = () => {
  ...
  const loadTopics = async () => {
    // Note: Assuming 'profiles' are linked via foreign key in Supabase 
    // select('*, profiles(username)') might need adjustment based on your exact schema
    // Since we don't know the exact schema, we will try to fetch basic data first.
    // Ensure you have a 'profiles' table and foreign keys set up.
  ...
  const handleCreateTopic = async () => {
    ...
    if (error) {
      ...
      // Explanation for the "Muted" error the user experienced
      showAlert("Ошибка", "Не удалось создать тему. Возможно, вы 'замьючены' (отсутствуют RLS политики на INSERT в Supabase).");
    } else {
